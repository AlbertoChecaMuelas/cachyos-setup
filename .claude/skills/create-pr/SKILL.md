...
1. Invocar `/update-changelog --branch <base>`. En modo branch-scoped, el skill delega en `tools/release/update-changelog.sh`, que clasifica los commits de la rama (feat/feature → Añadido, fix → Corregido, refactor/perf/docs → Cambiado, chore/build/ci/other → omitido) y ANTEPONE un bloque `### Añadido/Cambiado/Corregido` bajo `## [Unreleased]` en CHANGELOG.md, deduplicando bullets idénticos en [Unreleased] (idempotente: re-ejecutar `/create-pr` no duplica contenido en el changelog). La sección NO se renombra aquí — tras el merge de la PR y la creación del tag, la CI `auto-tag` la cierra como `[vX.Y.Z] - YYYY-MM-DD` y pushea el commit de cierre a `master`.
